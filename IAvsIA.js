document.addEventListener("DOMContentLoaded", function() {
	...
	/*function couldMultWin(joueur) {
		...
		for(var y = 0; y < 7; y++) {
			if(columnIsFull(y) == false) {
				...
				responseID = couldWinTest(firstEmptyCase(y), y, joueur);
				if(responseID != false) {
					putToken(responseID[1], responseID[1], joueur);
					if(isOver() == 1 || isOver() == 2) {
						copyGrid("restore");
						putToken(firstEmptyCase(y), y, "IA");
						if(canWinTest("humain") != false)
	...
	function IA() {
		...
		else if(couldMultWin("IA") == true) {console.log("in couldMultWin(IA)");}
		else if(couldMultWin("humain") == true) {console.log("in couldMultWin(humain)");}
		...
		else if(align2() == true) {console.log("in align2()");}
